...
public class FinanceInfoServiceImpl implements IFinanceInfoService {
	...
	public BaseRespBean addFinance(FinanceFlowReqBean reqBean, HttpSession httpSession) {
		...
		String toPath = httpSession.getServletContext().getRealPath("uploadFile/common/");
		toPath = toPath + ScfUUID.generate() + ".pdf";
		...
		try {
			...
			warningInfo.setWarnStatus("0");// 未阅
			warningInfo.setWarnMsg(toPath);
		} catch (Exception e) {
			log.error(e.getMessage());
		}
