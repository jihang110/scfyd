		...
		'click .mod': function (e, value, row, index) {
			...
			if(row.procdefName=="企业信息管理"||row.procdefName == '合同管理'
				...
				/*if(row.taskDefKey=="usertask1"){
					...
					//进入审核
					$('#mainFrame',top.document).attr('src','project/agencyCommon/agencyCommon.html?taskId='+row.taskId+'&procInstId='+row.procInstId+'&taskDefKey='+row.taskDefKey+'&procdefKey='+row.procdefKey);
				//}
